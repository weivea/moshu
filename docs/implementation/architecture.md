...
client 不提供绕过 agents server 的 executor RPC，executor 也不直接读取 client 状态、产品 DB 或 Pi Session JSONL。
...
executor 不拥有产品 DB/Pi Session JSONL、Agent definitions、Agent runtime、Provider 访问、Provider/model
...
- agents server 重启后从产品 DB 与 Pi Session JSONL 恢复；client/executor 使用稳定 ID 和新的
...
- Provider credential 从 app-owned `SecretVaultCredentialStore` 按 Run scope 读取，不进入 client 或 executor。
