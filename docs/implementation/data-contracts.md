...
- **配对 fail-closed（ingress 未就绪）**：`pairing.create` 仅在 mobile ingress ready 且有 exact public URL 时创建；否则抛稳定 `MOBILE_INGRESS_NOT_READY` 且**不创建/消耗任何 pairing 记录**。Desktop 创建按钮 `disabled` 直到 `mobileAccess.status.ingressReady && publicUrl`，避免产生无 QR 的失效配对。
