...
| 稳定角色 ID | `clientId`、`runtimeBoxId` 跨重连/重启保留 |
...
| `runtime_boxes` | id, name, kind, availability, last_seen_at | 稳定 `runtimeBoxId`；availability 包含 syncing/online/offline |
| `role_instances` | stable_id, role, instance_id, generation, connected_at, disconnected_at | 注册历史；不保存 socket 对象 |
| `agent_definitions` | id, name, current_version_id, disabled_at | Agent 定义全局共享，不直接绑定 Box |
...
| `mobile_attention_events` | seq PK（每 feed 单调）, event_id UNIQUE, dedupe_key UNIQUE, type, session_id?, run_id?, approval_id?, title_key, body_key, created_at_ms | Layer 5 durable attention feed；**仅脱敏 opaque id + generic 本地化键**，`dedupe_key` 保证同业务事件幂等 append |
...
| `mobile_attention_feed_meta` | id PK, next_seq, pruned_through_seq | seq high-water + retention 水位；cursor ≤ `pruned_through_seq` 触发 `resyncRequired`（bounded prune by age + per-feed 上限） |
...
- 只有 `client <-> agents server` 和 `agents server <-> Runtime Box` 两类连接。
...
Layer 5 在 §9.3/§9.4 之上新增 **Agent Server 持有** 的移动端未读/attention feed 合同与消费（见 `packages/contracts/src/mobile.ts`、`apps/agents-server/src/mobile-attention-projection.ts` + `product-rpc.ts`、`packages/database/src/mobile-attention-repository.ts`、`apps/mobile/src/rpc/attention-controller.ts`）。**无云 Push Relay、无 APNs remote/silent push、无后台伪保活、设备不落业务数据。**

- **`MobileAttentionEvent`（versioned，脱敏）**：`{ schemaVersion:1, eventId(uuid), seq(正整数、每 feed 单调), type ∈ approval_required|run_completed|run_failed|run_cancelled, visibility:"mobile-client", sessionId?/runId?/approvalId?（opaque）, createdAt, titleKey/bodyKey（generic 本地化键，如 `attention.approvalRequired.title`）}`，strict。**绝不含** prompt/message/tool raw args/provider secret/path 正文/shell command。
- **append（幂等、事务、重启不丢）**：Agent Server 在 approval 进入 `pending`、Run 达终态时投影出脱敏 append；`dedupeKey`（如 `approval:<id>` / `run:<id>`）保证同一业务事件不重复。`mobile-attention-projection.test.ts` 断言脱敏与去重。
- **每设备 ack cursor（read state，server 侧）**：`mobile.attention.list`（`{ cursor?, limit }` → `{ items, unreadCount, ackSeq, latestSeq, resyncRequired, nextCursor? }`，cursor 分页）与 `mobile.attention.ack`（`{ seq }`，CAS + 幂等 + 单调，旧 ack 不回退 cursor）。**peer identity 由 auth context 决定**，禁止伪造 clientId 或回退。均加入 mobile-client strict allowlist（`mobileClientProductRequestMethods`）。
- **bounded retention + resync**：按 age（如 30 天）与 per-feed 上限安全清理；cursor 过旧则 `list` 返回 `resyncRequired:true`（resnapshot），**不伪装无未读**。设备 revoke 后不可读；unpair 后 `deleteAckCursor` 清 read state，旧 feed 不泄漏给新 binding。DB 层用例见 `packages/database/tests/mobile-attention-repository.test.ts`（monotonic/idempotent、pagination、retention prune by age/count、malformed cursor 拒绝、revoke 清 cursor）。
