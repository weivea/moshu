...
> **Layer 2 实现状态（已落地）**：用户级 Tool/Action 审批已从设计变为真实实现（`ApprovalService` + 注入 `DurableActionAuthorizationService` 的执行门，见 §9.0.1 架构文档）。风险 tier 由 agents-server 用 `@moshu/action-broker` 从 Tool identity + 校验后参数**权威计算**，绝不信任模型/Tool wrapper/Runtime Box 自报；审批 request 与 Session Allow-all 策略 durable 持久（`action_approval_requests`、`session_approval_policies`），带 revision/CAS 与唯一 idempotency key；request 处于 pending 期间**不签发/消费 grant、不调用 Runtime Box**。Desktop 客户端已提供 Chat 等待审批卡片与全局待办面板；同一 Session 的多 client 通过事件实时同步，竞争由 server CAS 解决（一个 `applied`，其余 `superseded`）。审批 summary 只含脱敏 command/path/operation 与 redacted 参数键，绝不广播原始 command/env 或 secret；**shell/bash 的 command 预览完全 fail-closed：对命令完全不解析，无论输入如何都恒为固定常量 `shell [arguments hidden]`（不推导可执行文件/basename/首 token），Tool 字段已单独标识 `bash`；绝不公开任何 argv/flag value/URL/env**。

> **Layer 3 实现状态（已落地）**：新增**独立 Mobile ingress**（固定 loopback listener + `/mobile` 路径），与 Product RPC、Runtime ingress 物理/逻辑隔离，**绝不 fallback**；有独立 frame/body/inflight/handshake timeout、未认证连接与 HTTP 容量、per-source 限流与流量计量。配对经 Desktop 展示的二维码：QR payload 只含 mobile public URL、pairingId、一次性 code、agentServerId、server 公钥/指纹、有效期、协议区间，**绝不含 server secret 或长期 token，也不写日志/localStorage**，组件 unmount/过期即清理。配对 code ≥128-bit 熵、5 分钟、**server 只存 hash、single-use**，claim token 同样只存 hash。设备认证用 canonical Ed25519 SPKI 公钥/指纹 + `AgentServerIdentity` 签名 challenge（domain-tag 隔离，绑定 agentServerId、mobileClientId、deviceKeyId、instanceId、persisted generation、challengeId/nonce、协议版本、`relay-tls` transportSecurity）；WebSocket upgrade 前校验签名/激活 key/吊销/challenge 单次与过期/server identity，返回 role=`mobile-client`，RPC hello 必须 exact match。独立持久 generation high-water fence 阻止旧 generation/instance/late connection 复活，**设备吊销立即关闭匹配 peer**。Mobile ingress 走**独立 allowlist**（只读+受控 MVP 方法），Provider/Remote Access 控制/Runtime 配对/MCP/Skills/Project mutation/diagnostics/任意 DB 查询一律 deny；client preference/decision source 由 authenticated peer identity 派生，**不接受伪造 clientId**；事件按已订阅可见 Session 严格 Zod + redaction 下发。**只有本地 Desktop Product RPC 能 create/approve/revoke，Mobile ingress 不能自批**。传输边界为 relay TLS + 应用设备签名，**Noise 后置且不谎称已启用**；首版 Mobile client 只绑定一个 Agent Server（client 实现）、Desktop 必须在线、URL/server identity 变化需重新配对。

> **Layer 4 实现状态（已落地）**：真正可构建的 iPhone App = Capacitor Web UI（`apps/mobile`：React + Vite + TS strict + HeroUI + HashRouter）+ 原生 Swift 安全传输 plugin（`MoshuMobileTransport`）。**私钥/长期凭据绝不进入 JS**：CryptoKit Curve25519（Ed25519，**软件 key，非 Secure Enclave**）在原生生成，private key 存 Keychain `WhenUnlockedThisDeviceOnly` 且**不同步 iCloud、不入 UserDefaults/localStorage**；公钥用 canonical SPKI DER。**单 Server binding**（exact URL/agentServerId/server 公钥指纹/协议），已绑定拒绝覆盖，显式 unpair 才清 Keychain binding+私钥并关 socket。配对解析 versioned 二维码 → URLSession claim/轮询 status → 核对 server 公钥指纹后原子持久化；**code/claimToken/key 绝不输出到日志/error**，二维码扫描结果只在内存。重连持久单调 generation、每连接新 instanceId，验证 Agent Server challenge 签名后用设备 key 签 canonical upgrade payload，经 `URLSessionWebSocketTask` 带 `x-moshu-*` headers 连 WSS——**长期凭据只走 header/签名、不入 query string**；TLS 只信系统信任链（relay TLS 可见，不错误 pin relay cert），帧按 connectionId+单调 sequence、限帧/队列大小、拒 binary、丢弃旧连接。JS 侧 browser-safe `@moshu/process-rpc-core` 完成 hello/ack 与 expected server identity，identity mismatch 立即 close/清 volatile state，不自动改 pin；**业务数据仅存 React 内存**（Session/Project/message/approval 绝不落 localStorage/Preferences/Keychain），断线即清空只显示 offline/reconnect，**不缓存、不离线排队、不 silent fallback 伪造 connected**；仅 appearance/language 可持久化。Capacitor iOS 工程 **Info.plist 仅申请相机（二维码）**、**无宽泛 ATS、无 Local Network/Bonjour**；不写死个人签名凭据/Team ID。canonical payload 有共享 test vectors 证明 Swift/TS 字节一致（CryptoKit Ed25519 签名随机化，故只断言跨实现验签）。**移动通知后台/suspended 可靠投递与发布加固仍属 Layer 5**。
>
