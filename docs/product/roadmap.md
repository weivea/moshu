...
- 自定义 Agent、MCP、Skills、知识库和 Canvas 可并行开发，但必须复用同一套权限和事件模型，并遵守 server/Runtime Box 各自唯一持久化所有者。
- 架构交付固定按 RPC/binary POC -> agents-server extraction -> Runtime Box/Agent registry -> Tool Bridge/Action Broker -> MCP/Skills -> recovery/release hardening 推进。
...
- Electrobun client、agents server、Runtime Box companion skeleton；WebSocket + versioned JSON RPC。
...
- 未经 server Policy/approval/intent 和一次性 grant，Runtime Box 不执行。
- 一个带文件修改与审批的 Run 可完整持久化和回放，命令进程树可取消/清理。
- `SessionManager` 通过当前 schema、WAL、关闭重开和 server restart 测试。
- Runtime Box offline 时绑定 Agent 不能启动新 Run。
- Runtime Box 注册/重连后 full snapshot 原子替换完成前保持 syncing，绑定 Agent 不 runnable。
- MCP/Skill command 只有 owning Runtime Box 持久化后成功；server 产品数据无 recoverable config/content/credential copy。
...
- local Runtime Box root/credential file 的 `0700`/`0600`、owner、atomic replace、symlink/no-follow gate 通过。
...
- 受控命令执行、风险分类和审批。
- 每个实际 Tool 由 server 签发一次性 grant，Runtime Box 验证后执行。
...
3. 用户允许后，server 持久化决定/intent 并签发一次性 grant；Runtime Box 验证后执行，结果回到 server 轨迹。
...
5. local Runtime Box 离线后，绑定 Agent 的新 Run 被阻止；恢复注册后才可继续启动。

### E2E-07：崩溃恢复

1. Run 完成部分文件操作后分别强制结束 agents server 与 Runtime Box。
2. client 按 capped backoff 监管重启；达到上限时停止 crash loop 并显示恢复 UX。
3. 重连后显示最后已持久化状态、已完成动作和 Runtime Box registry 状态。
4. 恢复不重复已成功的副作用工具；旧 instance/generation 的结果被拒绝。
5. 状态不确定的动作要求人工确认。

### E2E-08：自定义 Agent

1. 创建包含固定模型、assigned Runtime Box MCP/Skill stable refs、知识、权限和 Runtime Box binding 的 Agent。
2. 不兼容模型或缺失依赖在保存/运行前可见。
3. owning Runtime Box 生成可选 Skill bundle；导出不含密钥、本地绝对路径或 server-side Skill copy。
4. 在另一安装中先把资源导入 selected Runtime Box、再保存 Agent refs，修复依赖后可运行。
...
3. Runtime Box 保存 immutable version/content/hash；Agent 只引用 assigned Runtime Box stable resource version。
...
6. resources/scripts 仍通过 Runtime Box，Skill 脚本遵守命令权限和 execution grant。
...
- server 持久化 Policy/approval/intent；Runtime Box 拒绝过期、重复、篡改或目标不匹配的 grant。
- Provider/model credential 不进入 Runtime Box；MCP credential 只在 owning Runtime Box private store 与目标 connection/process 可达，不进入 server copy、query/UI/log/export、全局环境或无关 child/Agent。
...
| --- | --- | --- | --- | --- |
| Client/server/Runtime Box、RPC、registry、supervisor | A0–A2 | A3 + A5 完整 | A4 扩展 | 远程/跨平台 |
...
| MCP | Runtime Box ownership/RPC/private-secret POC | 无正式 UI | routed UI + epoch/revision reconciliation + Runtime Box config/auth/lifecycle | Resources/Prompts/企业 |
| Skills | immutable Runtime Box store/ref POC | 无正式 UI | routed UI + inventory delta/tombstone + prompt fetch/resources/scripts | Git 更新/市场 |
...
| stable ID、instance/generation 和 Runtime Box lease 细节 | A0–A2 |
